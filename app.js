...
/* V5.5 - Inventario integrato esclusivamente nel gestionale */
const INVENTORY_KEY="dpz_inventory_v55";
const INVENTORY_SEED=[
 {cat:"Affettati",name:"Prosciutto crudo",unit:"kg"},{cat:"Affettati",name:"Prosciutto cotto",unit:"kg"},{cat:"Affettati",name:"Salame",unit:"kg"},{cat:"Affettati",name:"Bresaola",unit:"kg"},{cat:"Affettati",name:"Mortadella",unit:"kg"},{cat:"Affettati",name:"Speck",unit:"kg"},
 {cat:"Formaggi",name:"Mozzarella",unit:"kg"},{cat:"Formaggi",name:"Fior di latte",unit:"kg"},{cat:"Formaggi",name:"Gorgonzola",unit:"kg"},{cat:"Formaggi",name:"Grana",unit:"kg"},{cat:"Formaggi",name:"Provola",unit:"kg"},{cat:"Formaggi",name:"Scamorza",unit:"kg"},{cat:"Formaggi",name:"Pecorino",unit:"kg"},
 {cat:"Pizzeria",name:"Farina",unit:"kg"},{cat:"Pizzeria",name:"Lievito",unit:"pz"},{cat:"Pizzeria",name:"Passata di pomodoro",unit:"conf."},{cat:"Pizzeria",name:"Pomodori pelati",unit:"conf."},{cat:"Pizzeria",name:"Olio EVO",unit:"l"},{cat:"Pizzeria",name:"Olio semi",unit:"l"},{cat:"Pizzeria",name:"Olive nere",unit:"conf."},{cat:"Pizzeria",name:"Capperi",unit:"conf."},{cat:"Pizzeria",name:"Origano",unit:"conf."},
 {cat:"Verdure",name:"Rucola",unit:"kg"},{cat:"Verdure",name:"Pomodori",unit:"kg"},{cat:"Verdure",name:"Melanzane",unit:"kg"},{cat:"Verdure",name:"Zucchine",unit:"kg"},{cat:"Verdure",name:"Peperoni",unit:"kg"},{cat:"Verdure",name:"Cipolla rossa",unit:"kg"},{cat:"Verdure",name:"Funghi",unit:"kg"},{cat:"Verdure",name:"Patate",unit:"kg"},{cat:"Verdure",name:"Insalata",unit:"kg"},
 {cat:"Pesce",name:"Calamari",unit:"kg"},{cat:"Pesce",name:"Gamberi",unit:"kg"},{cat:"Pesce",name:"Cozze",unit:"kg"},{cat:"Pesce",name:"Vongole",unit:"kg"},{cat:"Pesce",name:"Polpo",unit:"kg"},{cat:"Pesce",name:"Pesce spada",unit:"kg"},{cat:"Pesce",name:"Tonno",unit:"kg"},{cat:"Pesce",name:"Alici",unit:"kg"},{cat:"Pesce",name:"Salmone",unit:"kg"},
 {cat:"Carne",name:"Hamburger",unit:"pz"},{cat:"Carne",name:"Salsiccia",unit:"kg"},{cat:"Carne",name:"Petto di pollo",unit:"kg"},{cat:"Carne",name:"Porchetta",unit:"kg"},{cat:"Carne",name:"Wurstel",unit:"pz"},
 {cat:"Dispensa",name:"Pasta",unit:"kg"},{cat:"Dispensa",name:"Riso",unit:"kg"},{cat:"Dispensa",name:"Sale fino",unit:"kg"},{cat:"Dispensa",name:"Sale grosso",unit:"kg"},{cat:"Dispensa",name:"Zucchero",unit:"kg"},{cat:"Dispensa",name:"Pangrattato",unit:"kg"},{cat:"Dispensa",name:"Uova",unit:"pz"},{cat:"Dispensa",name:"Burro",unit:"kg"},{cat:"Dispensa",name:"Panna",unit:"l"},
 {cat:"Bevande",name:"Acqua naturale vetro",unit:"bottiglie"},{cat:"Bevande",name:"Acqua frizzante vetro",unit:"bottiglie"},{cat:"Bevande",name:"Coca-Cola",unit:"bottiglie"},{cat:"Bevande",name:"Fanta",unit:"bottiglie"},{cat:"Bevande",name:"Sprite",unit:"bottiglie"},{cat:"Bevande",name:"Birra",unit:"bottiglie"},{cat:"Bevande",name:"Vino bianco",unit:"bottiglie"},{cat:"Bevande",name:"Vino rosso",unit:"bottiglie"},
 {cat:"Pulizia",name:"Detersivo piatti",unit:"pz"},{cat:"Pulizia",name:"Sgrassatore",unit:"pz"},{cat:"Pulizia",name:"Candeggina",unit:"pz"},{cat:"Pulizia",name:"Carta cucina",unit:"pz"},{cat:"Pulizia",name:"Guanti",unit:"conf."},{cat:"Pulizia",name:"Sacchi spazzatura",unit:"conf."},{cat:"Pulizia",name:"Tovaglioli",unit:"conf."}
].map((x,i)=>({id:"inv-"+(i+1),qty:0,min:1,note:"",...x}));
let inventoryItems=[];let inventoryCat="Tutti";
function loadInventory(){try{inventoryItems=JSON.parse(localStorage.getItem(INVENTORY_KEY)||"null")||INVENTORY_SEED}catch(e){inventoryItems=[...INVENTORY_SEED]}}
function saveInventory(){localStorage.setItem(INVENTORY_KEY,JSON.stringify(inventoryItems));renderInventory()}
function inventoryCats(){return [...new Set(inventoryItems.map(x=>x.cat).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"it"))}
function renderInventory(){const grid=$("#inventoryGrid");if(!grid)return;const cats=inventoryCats(),q=($("#inventorySearch")?.value||"").trim().toLowerCase();if(inventoryCat!=="Tutti"&&!cats.includes(inventoryCat))inventoryCat="Tutti";$("#inventoryFilters").innerHTML=["Tutti",...cats].map(c=>`<button class="${inventoryCat===c?"active":""}" data-inv-cat="${esc(c)}">${esc(c)}</button>`).join("");let list=inventoryItems.filter(x=>(inventoryCat==="Tutti"||x.cat===inventoryCat)&&(!q||x.name.toLowerCase().includes(q)||x.cat.toLowerCase().includes(q)));const low=inventoryItems.filter(x=>Number(x.qty)<=Number(x.min)).length;$("#inventorySummary").innerHTML=`<div><b>${inventoryItems.length}</b><span>Voci totali</span></div><div><b>${cats.length}</b><span>Categorie</span></div><div><b>${low}</b><span>Da riordinare</span></div>`;grid.innerHTML=list.length?list.map(x=>{const isLow=Number(x.qty)<=Number(x.min);return`<article class="inventory-card ${isLow?"low":""}"><div class="inventory-card-head"><div><h3>${esc(x.name)}</h3><small>${esc(x.cat)}${x.note?` · ${esc(x.note)}`:""}</small>${isLow?`<div class="inventory-low-label">SCORTA BASSA</div>`:""}</div><div class="inventory-qty">${Number(x.qty).toLocaleString("it-IT")} <small>${esc(x.unit)}</small></div></div><div class="inventory-stepper"><button data-inv-minus="${x.id}">−</button><button data-inv-set="${x.id}" class="secondary">IMPOSTA</button><button data-inv-plus="${x.id}">+</button></div><button class="inventory-edit" data-inv-edit="${x.id}">MODIFICA VOCE</button></article>`}).join(""):`<div class="empty">Nessuna voce trovata</div>`;document.querySelectorAll("[data-inv-cat]").forEach(b=>b.onclick=()=>{inventoryCat=b.dataset.invCat;renderInventory()});document.querySelectorAll("[data-inv-minus]").forEach(b=>b.onclick=()=>changeInventoryQty(b.dataset.invMinus,-1));document.querySelectorAll("[data-inv-plus]").forEach(b=>b.onclick=()=>changeInventoryQty(b.dataset.invPlus,1));document.querySelectorAll("[data-inv-set]").forEach(b=>b.onclick=()=>setInventoryQty(b.dataset.invSet));document.querySelectorAll("[data-inv-edit]").forEach(b=>b.onclick=()=>openInventoryEditor(b.dataset.invEdit))}
function changeInventoryQty(id,delta){const x=inventoryItems.find(i=>i.id===id);if(!x)return;x.qty=Math.max(0,Number(x.qty||0)+delta);saveInventory()}
function setInventoryQty(id){const x=inventoryItems.find(i=>i.id===id);if(!x)return;const v=prompt(`Quantità di ${x.name}`,String(x.qty));if(v===null)return;const n=Number(String(v).replace(",","."));if(Number.isNaN(n)||n<0)return toast("Quantità non valida");x.qty=n;saveInventory()}
function openInventoryEditor(id){const x=inventoryItems.find(i=>i.id===id);$("#inventoryId").value=x?.id||"";$("#inventoryName").value=x?.name||"";$("#inventoryCategory").value=x?.cat||(inventoryCat!=="Tutti"?inventoryCat:"");$("#inventoryQty").value=x?.qty??0;$("#inventoryUnit").value=x?.unit||"pz";$("#inventoryMin").value=x?.min??1;$("#inventoryNote").value=x?.note||"";$("#inventoryCategoryList").innerHTML=inventoryCats().map(c=>`<option value="${esc(c)}"></option>`).join("");$("#inventoryModalTitle").textContent=x?"Modifica voce":"Nuova voce inventario";$("#deleteInventoryItem").style.display=x?"block":"none";$("#inventoryModal").classList.add("open")}
function saveInventoryEditor(){const id=$("#inventoryId").value,name=$("#inventoryName").value.trim(),cat=$("#inventoryCategory").value.trim();if(!name||!cat)return toast("Inserisci nome e categoria");let x=inventoryItems.find(i=>i.id===id);if(!x){x={id:"inv-"+Date.now()};inventoryItems.push(x)}Object.assign(x,{name,cat,qty:Math.max(0,Number($("#inventoryQty").value||0)),unit:$("#inventoryUnit").value,min:Math.max(0,Number($("#inventoryMin").value||0)),note:$("#inventoryNote").value.trim()});closeModal("#inventoryModal");saveInventory();toast("Inventario aggiornato")}
function deleteInventoryEditor(){const id=$("#inventoryId").value,x=inventoryItems.find(i=>i.id===id);if(!x||!confirm(`Eliminare ${x.name} dall'inventario?`))return;inventoryItems=inventoryItems.filter(i=>i.id!==id);closeModal("#inventoryModal");saveInventory();toast("Voce eliminata")}
function exportInventory(){const rows=[["Categoria","Prodotto","Quantità","Unità","Soglia minima","Note"],...inventoryItems.map(x=>[x.cat,x.name,x.qty,x.unit,x.min,x.note])];const csv=rows.map(r=>r.map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(";")).join("\n");const a=document.createElement("a");a.href=URL.createObjectURL(new Blob(["\ufeff"+csv],{type:"text/csv;charset=utf-8"}));a.download=`inventario_duepuntozero_${new Date().toISOString().slice(0,10)}.csv`;a.click();URL.revokeObjectURL(a.href)}
loadInventory();renderInventory();
if($("#inventorySearch"))$("#inventorySearch").oninput=renderInventory;
if($("#inventoryAdd"))$("#inventoryAdd").onclick=()=>openInventoryEditor();
if($("#inventoryExport"))$("#inventoryExport").onclick=exportInventory;
if($("#closeInventoryModal"))$("#closeInventoryModal").onclick=()=>closeModal("#inventoryModal");
if($("#saveInventoryItem"))$("#saveInventoryItem").onclick=saveInventoryEditor;
if($("#deleteInventoryItem"))$("#deleteInventoryItem").onclick=deleteInventoryEditor;
